...
function parseSearchParams(searchParams: Record<string, unknown>): ResumeFilters {
  const filters: ResumeFilters = {};
  
  // Parse focus areas
  if (searchParams.focus && typeof searchParams.focus === 'string') {
    filters.focus = searchParams.focus.split(',').filter(f => 
      ['frontend', 'backend', 'fullstack', 'leadership', 'product', '3d-graphics', 'realtime'].includes(f)
    ) as FocusArea[];
  }
  
  // Parse technologies
  if (searchParams.technologies && typeof searchParams.technologies === 'string') {
    filters.technologies = searchParams.technologies.split(',').map(t => t.trim());
  }
  
  // Parse domains
  if (searchParams.domains && typeof searchParams.domains === 'string') {
    filters.domains = searchParams.domains.split(',').filter(d =>
      ['frontend', 'backend', 'infrastructure', '3d', 'payments', 'realtime', 'auth', 'marketplace', 'social', 'testing', 'devops'].includes(d)
    ) as Domain[];
  }
  
  // Parse role levels
  if (searchParams.role && typeof searchParams.role === 'string') {
    filters.role = searchParams.role.split(',').filter(r =>
  ...
  if (searchParams.experience && typeof searchParams.experience === 'string') {
    filters.experience = searchParams.experience.split(',').filter(e =>
  ...
  if (searchParams.format && typeof searchParams.format === 'string') {
    if (['web', 'pdf', 'minimal', 'detailed'].includes(searchParams.format)) {
      filters.format = searchParams.format as 'web' | 'pdf' | 'minimal' | 'detailed';
    }
  }
...
function filterProjects(projects: ProjectContribution[], filters: ResumeFilters): ProjectContribution[] {
  return projects
    .filter(project => {
      // Filter by focus areas
      if (filters.focus && filters.focus.length > 0) {
        if (!filters.focus.some(focus => project.focusAreas.includes(focus))) {
          return false;
        }
      }
      ...
      if (filters.domains && filters.domains.length > 0) {
        if (!filters.domains.some(domain => project.domains.includes(domain))) {
          return false;
        }
      }
      ...
      if (filters.technologies && filters.technologies.length > 0) {
        ...
        if (!filters.technologies.some(tech => 
      ...
      if (filters.years) {
        ...
        const projectYears = project.timeline.match(/\d{4}/g)?.map(y => parseInt(y, 10)) || [];
...
function filterAchievements(projects: ProjectContribution[], filters: ResumeFilters): Achievement[] {
  ...
  return allAchievements
    .filter(achievement => {
      // Filter by domains
      if (filters.domains && filters.domains.length > 0) {
        if (!filters.domains.some(domain => achievement.domains.includes(domain))) {
          return false;
        }
      }
...
function filterSkills(skills: SkillCategory[], filters: ResumeFilters): SkillCategory[] {
  return skills
    .filter(skillCategory => {
      // Filter by domains
      if (filters.domains && filters.domains.length > 0) {
        if (!filters.domains.some(domain => skillCategory.domains.includes(domain))) {
          return false;
        }
      }
...
function generateSummary(filters: ResumeFilters): string {
  ...
  if (filters.focus && filters.focus.length > 0) {
    if (filters.focus.includes('3d-graphics')) {
      customSummary = 'Full-stack developer specializing in 3D graphics and real-time animation systems. Expert in Three.js, WebGL, and motion capture technology with experience building production-scale 3D applications.';
    } else if (filters.focus.includes('realtime')) {
      customSummary = 'Full-stack developer specializing in real-time systems and modern web architecture. Expert in WebSocket technologies, optimistic UI updates, and event-driven systems.';
    } else if (filters.focus.includes('leadership')) {
      customSummary = 'Technical leader and architect with experience founding and scaling software platforms. Proven track record of building high-performance teams and delivering complex technical products.';
    } else if (filters.focus.includes('backend')) {
      customSummary = 'Backend-focused developer with expertise in scalable API design, database architecture, and cloud infrastructure. Experienced in building robust systems that handle complex business logic.';
    } else if (filters.focus.includes('frontend')) {
      customSummary = 'Frontend developer with strong UI/UX design skills and expertise in modern React ecosystems. Specialized in building intuitive, performance-optimized user experiences.';
    }
  }
...
export function useResumeFilter() {
  ...
  const filteredData = useMemo(() => {
    ...
    const effectiveFilters = {
      ...filters,
      focus: filters.focus?.length ? filters.focus : resumeProfile.defaults.focusAreas,
      priority: filters.priority ?? 6 // Default minimum priority
    };
    
    const filteredProjects = filterProjects(resumeProfile.projects, effectiveFilters);
    const filteredSkills = filterSkills(resumeProfile.skills, effectiveFilters);
    const topAchievements = filterAchievements(filteredProjects, effectiveFilters);
