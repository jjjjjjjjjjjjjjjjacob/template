...
function createAppConfig() {
  ...
  const appName = String(env.VITE_APP_NAME || 'template');
  const appDomain = String(env.VITE_APP_DOMAIN || 'localhost');
  ...
  const appTwitterHandle = String(env.VITE_APP_TWITTER_HANDLE || '@template');
  ...
  return {
    ...
    // Display names
    displayName: appName,
    tagline: 'modern full-stack template',
    description: `${appName} is a modern full-stack monorepo template built with TanStack Start, Convex, and Three.js.`,

    // SEO defaults
    seo: {
      defaultTitle: `${appName} | modern full-stack template`,
      titleTemplate: `%s | ${appName}`,
      defaultDescription: `${appName} is a modern full-stack monorepo template built with TanStack Start, Convex, and Three.js.`,
    },
    ...
    env: {
      VITE_APP_NAME: String(env.VITE_APP_NAME || 'template'),
      VITE_APP_DOMAIN: String(env.VITE_APP_DOMAIN || 'localhost'),
      VITE_APP_SUBDOMAIN: env.VITE_APP_SUBDOMAIN
        ? String(env.VITE_APP_SUBDOMAIN)
        : undefined,
      VITE_APP_TWITTER_HANDLE: String(
        env.VITE_APP_TWITTER_HANDLE || '@template'
      ),
