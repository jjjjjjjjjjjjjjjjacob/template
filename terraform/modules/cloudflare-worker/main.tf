...
resource "cloudflare_workers_script" "frontend" {
  ...
  script_name = "template-${var.environment == "ephemeral" ? var.prefix : var.environment}-frontend"
...
# The route that triggers the worker (e.g., example.com/*)
...
resource "cloudflare_dns_record" "frontend_a" {
  ...
  type    = "A"
  proxied = true
  comment = "Managed by Terraform for template Worker"
  ttl     = 1
}
