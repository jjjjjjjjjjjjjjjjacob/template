  ...
  handler: async (ctx, args) => {
          ...
          if (passesFilters) {
            ...
            const creator = await ctx.db
              ...
              .withIndex('byExternalId', (q) =>
                q.eq('external_id', vibe.createdById)
              )
              .first();
        ...
        if (mergedFilters.emojiRatings && passesFilters) {
          ...
          if (emojis && emojis.length > 0) {
            ...
            const userRatings = await ctx.db
              .query('ratings')
              .withIndex('user', (q) => q.eq('userId', user.external_id))
              .collect();
    ...
    if (!includeTypes || includeTypes.includes('review')) {
      ...
      for (const rating of allRatings) {
        ...
        const reviewMatch = rating.review
          ? fuzzyMatch(rating.review, searchText)
          : false;

        // Check for excluded terms
        const hasExcludedTerm = rating.review
          ? parsedQuery.excludedTerms.some((term) =>
        ...
        if (reviewMatch || !searchQuery.trim()) {
          ...
          if (passesFilters) {
            ...
            const reviewer = await ctx.db
              .query('users')
              .withIndex('byExternalId', (q) =>
                q.eq('external_id', rating.userId)
              )
              .first();
